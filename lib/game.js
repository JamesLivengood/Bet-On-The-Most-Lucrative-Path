...
class Game {
  ...
  async setup() {
    ...
    this.slowCanvas = document.getElementsByClassName("slow-canvas")[0];
    this.slowCanvas.width = 464-19;
    this.slowCanvas.height = 464-19;
    this.ctxSlow = this.slowCanvas.getContext("2d");
    this.slowGrid = new Grid([0, 6], [6, 0], 7, this.ctxSlow, 64);
    this.slowGrid.populateGrid();
    this.slowGrid.drawGrid(7);
    this.slowDfsBuilder = new dfsBuildMaze(this.slowGrid, 64, 'left');
    this.slowDfsBuilder.speed = 500;
    this.slowDfsBuilder.drawMaze(this.ctxSlow);
    ...
    // this.nodeTree.fillStyle = 'black';
    this.nodeTreeCtx.fillText('Node tree diagram code in progress, please check in a couple days :)', 100, 100);
