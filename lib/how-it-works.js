...
const howItWorks = () => {
  ...
  slowCanvas.width = 464-14;
  slowCanvas.height = 464-14;
  const ctxSlow = slowCanvas.getContext("2d");
  // const slowGrid = new Grid([0, 11], [11, 0], 12, const ctxSlow, 32);
  const slowGrid = new Grid([0, 6], [6, 0], 7, ctxSlow, 64);
  slowGrid.populateGrid();
  slowGrid.drawGrid(12);
  const slowDfsBuilder = new dfsBuildWithTree(slowGrid, 64, 'left');
  slowDfsBuilder.speed = 500;
